...
'''
import pandas as pd
from nlprules.radexpressions import string_search
...
def check_all_matches(candidate:str,
                      expression:list,
                      ) -> dict:
    """
    ...
    return {part.strip():string_search(candidate, part.strip(), return_bool=False) for part in expression}

def evaluate_sentences(candidate:str,
                       expression:list,
                       verbose:bool=False,
                        ) -> bool:
    """
...
def evaluate_logical_statement(candidate:str,
                               expression:str,
                               verbose:bool=False,
                               ) -> bool:
    """
    The main point of inpout to evaluate a logical statement.
    ...
    if isinstance(expression, str): # symbol or statement ot eval
        if expression in ['&', '|', '¬', '^', 'v', '!']: # Replace symbols with python operators
    ...
    if isinstance(expression, list):
        # Evaluate sub-statements recursively
        expression = [i for i in expression if i not in ['(', ')']] # Remove excess brackets
        sub_results = [evaluate_logical_statement(candidate, sub, verbose=verbose)
                       for sub in expression]

        # Evaluate subexpression
        sub_results_str = " ".join(str(elem) for elem in sub_results)
        if verbose:
            print(list_to_string(expression)[1:-1], '=>', sub_results_str, '=>', eval(sub_results_str))
        return eval(sub_results_str)

    raise ValueError("Invalid logical statement")
    
def search_dataframe(df:pd.DataFrame, 
                  column:str, 
                  expression:str, 
                  new_column_name:str=None, 
                  debug_column:bool=False,
                  sentencizer:bool=False) -> pd.DataFrame:
    """
    ...
    Args:
        ...
        new_column_name (str, optional): The new column name to store the results of the search. Defaults to None.
        debug_column (bool, optional): Return matching strings for regex search to enable debugging. Defaults to False.
        sentencizer (bool, optional): If True, split the column into sentences and search each sentence. Defaults to False.
    ...
    df[new_column_name] = df[column].map(lambda x: func(x, expression=expression))
  
    if debug_column:
        df[new_column_name+'_matches'] = df[column].map(lambda x: check_all_matches(x, expression))
