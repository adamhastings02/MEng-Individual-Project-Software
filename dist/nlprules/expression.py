...
"""
import re
from pyparsing import Word, alphanums, oneOf, ZeroOrMore, infixNotation, opAssoc
...
class Expression():
    ...
    def _get_grammar(self):
        ...
        expr = infixNotation(self.statement | '(' + ZeroOrMore(self.not_) + \
                             self.statement + ZeroOrMore(self.and_ | self.or_) + ')' |
    ...
    def check_input_string(self, input_str: str) -> bool:
        ...
        if not num_open_parentheses == num_close_parentheses:
            raise ValueError(f"Input search string has mismatching \
                             number of opening and closing parentheses \n {input_str}")

        for c in input_str:
            if not c in valid_chars:
                raise ValueError(f"Input search string contains \
